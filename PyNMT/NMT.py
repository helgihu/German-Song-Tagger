...
class EncoderLayer(NNModule):
   ...
   def __init__(self, input_size, rnn_size, dropout_rate):
      ...
      self.residualConnection = (input_size == 2*rnn_size)
      self.rnn = nn.LSTM(input_size, rnn_size, batch_first=True, bidirectional=True)
      self.dropout = nn.Dropout(dropout_rate)

   def forward(self, reprs, seq_len):
      packed_input = pack_padded_sequence(self.dropout(reprs), seq_len, batch_first=True)
...
class DecoderLayer(NNModule):
   ...
   def init_states(self, batch_size):
      self.hidden_states = self.zero_tensor(batch_size, self.rnn_size)
      self.cell_states   = self.zero_tensor(batch_size, self.rnn_size)
   ...
   def forward(self, reprs, ctx_vec):
      ...
      return reprs+self.hidden_states if self.residualConnection else self.hidden_states
...
class NMTDecoder(NNModule):
   ...
   def init_decoder(self, src_wordIDs, src_len):
      ...
      enc_context = self.zero_tensor(batch_size, self.ctx_size)
